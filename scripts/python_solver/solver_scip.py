...
# solver_highs.py
"""
HiGHS MIP Solver for Kubernetes Pod Scheduling Optimization.

This solver uses OR-Tools' HiGHS (High Performance Optimization Solver) mixed-integer
programming solver. It uses the SAME tiered lexicographic optimization approach as the
CP-SAT solver (solver_cp_sat.py):
...
Key advantages over CBC:
1. Generally faster on MIP problems
2. Lower memory footprint
3. MIT license (more permissive)

NOTE: Warm starts via SetHint() are DISABLED due to a segfault bug in OR-Tools 9.14.x
with HiGHS. The solver works correctly but may be slower than it could be with warm
starts. The SetHint() code is preserved and can be re-enabled when OR-Tools fixes
this issue. See: https://github.com/google/or-tools/issues (check for HiGHS SetHint bugs)

This solver uses the SAME Go contract as the CP-SAT solver (solver_cp_sat.py).
It can be used by setting SOLVER_TYPE=highs or SOLVER_PATH to point to this script.
"""
...
# HiGHS/MIP solver status codes mapped to the same strings as CP-SAT
...
# Binary variable threshold (HiGHS returns exact 0/1, but we use 0.5 for safety)
...
@dataclass(frozen=True)
class DecisionVars:
    """Decision variables for the HiGHS MIP model.

    Uses true binary integer variables (0 or 1) for exact solutions.
    No rounding is needed since HiGHS solves the integer program directly.
    """
...
#################################################
# --- HiGHS MIP Solver Class --------------------
#################################################

class HiGHSSolver:
    """Mixed Integer Programming solver using OR-Tools HiGHS for pod scheduling.
    
    Uses true binary integer variables for exact 0/1 solutions without rounding.
    Supports warm starts between optimization stages for faster solving.
    """
    ...
    def _status_str(cls, st: int) -> str:
        """Convert HiGHS solver status to string."""
        if not isinstance(st, int):
            raise TypeError(f"status must be int (HiGHS solver status), got {type(st).__name__}")
    ...
    def solve(self, instance: dict) -> dict:
        ...
        err = self._add_protected_stay_constraints(solver, problem, vars)
        if err is not None:
            elapsed = int((time.monotonic() - started_at) * 1000)
    ...
    def _create_solver(self, options: SolverOptions) -> Optional[pywraplp.Solver]:
        """Create and configure the HiGHS MIP solver."""
        # Try HiGHS first (preferred), fall back to CBC then SCIP if not available
    ...
    def _build_decision_vars(self, solver: pywraplp.Solver, problem: Problem) -> DecisionVars:
        ...
        Uses true binary integer variables (0 or 1) for exact solutions.
        HiGHS will solve the integer program directly without rounding.
        """
        placed = [
    ...
    def _solve_lexicographically(
        ...
        HiGHS-specific: Uses SetHint() for warm starting between stages,
        which can significantly speed up subsequent solves.
        ...
        def apply_warm_start():
            """Apply current solution as warm start hints for next solve.
            
            NOTE: SetHint() is disabled due to a segfault bug in OR-Tools' HiGHS
            wrapper when re-solving after adding constraints. The solver will
            still work correctly but may be slightly slower without warm starts.
            
            HiGHS supports MIP starts via SetHint(), which can significantly
            speed up subsequent solves by providing a good starting point.
            However, calling SetHint() before a re-solve after modifying the
            model (adding constraints) causes a segmentation fault in the
            underlying C++ code.
            
            TODO: Re-enable when OR-Tools fixes this issue.
            """
            # Disabled due to segfault - see docstring above
            pass
        ...
        def apply_current_placement_as_hint():
            ...
            NOTE: SetHint() is disabled due to a segfault bug in OR-Tools' HiGHS
            wrapper. See apply_warm_start() docstring for details.

            For each pod that is currently running on an eligible node, we would hint:
            - placed[i] == 1
            - assign[i][pos(orig_j)] == 1
            
            TODO: Re-enable when OR-Tools fixes the SetHint() issue.
            """
            # Disabled due to segfault - see docstring above
            pass
        ...
        #########################
        # Apply initial hints from current cluster state
        #########################
        ...
        for p in tiers:
            ...
            # Apply warm start for next stage
            apply_warm_start()
            ...
            if remaining_wall() > 1e-3 and rem_tiers > 1e-3:
                ...
                if running_ge:
                    ...
                    # Apply warm start for next tier
                    apply_warm_start()
    ...
    def _compute_overall_status(self, phases: list[dict], st: int) -> str:
        ...
        Note: HiGHS MIP solver provides exact integer solutions.
        """
